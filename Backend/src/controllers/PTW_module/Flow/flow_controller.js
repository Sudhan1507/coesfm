...
static async createFlowDetailsController(req, res) {
    ...
    if (!flowName || !steps || !Array.isArray(steps) || steps.length === 0) {
        return res.status(400).json({ error: 'flowName and steps are required.' });
    }

    try {
        const result = await FlowService.createFlowDetails(flowName, steps, createdBy);
...
static async updateFlowStepController(req, res) {
    try {
        ...
        const result = await FlowService.updateFlowStepService(flowId, stepNo, statusName, declaration, userId, updatedBy);
...
static async  addFlowStepController(req,res){
    ...
    try {
        const result = await FlowService.addFlowStepService(flowId, statusName, declaration, userId, createdBy);
