...
export default class PermitTypeDao{
    ...
    static async createPermitTypeDao(permitType){
        const sql='INSERT INTO m_permit_type (ptName,checklistId,flowId,reqId,remarks,active) VALUES(?,?,?,?,?,?)';
        const values = [
            permitType.ptName,
    ...
    static async updatePermitTypeDao(ptId,permitType){
        const sql='UPDATE m_permit_type SET ptName=?,checklistId=?, flowId=?, reqId=?, remarks=?, active=? where ptId=? ';
        const values = [
            permitType.ptName,
            permitType.checklistId,
            permitType.flowId,
