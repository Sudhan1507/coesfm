...
export default class PermitToWorkDao{
    ...
    static async addSignOff(appId, statusName, signOffRemarks, userId, signature) {
        ...
        if (appStatus === 'Completed') {
            ...
            return {
                success: false,
                message: 'The application is already completed and cannot be signed off.',
                appStatus
            };
        }
        ...
        static async updatePermitStatus(appId, workStatus, appStatus, updatedBy) {
            ...
            try {
                await db.execute(updatePermitSql, [workStatus, appStatus, updatedBy, appId]);
                return true;
            } catch (err) {
        ...
        static async updateChecklistResponse(appId, serialNo, checkOptions, remarks, updatedBy) {
            ...
            try {
                await db.execute(checklistResponseSql, [checkOptions, remarks, updatedBy, appId, serialNo]);
                return true;
            } catch (err) {
