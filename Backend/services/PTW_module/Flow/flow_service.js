...
export default class FlowService{
        ...
        try {
            ...
            for (let stepNo = 0; stepNo < steps.length; stepNo++) {
                ...
                if (!statusName || !declaration) {
                    throw new Error('Invalid parameters: All step parameters are required.');
                }

                await FlowDao.createFlowDetails(flowId, stepNo + 1, statusName, declaration, userId || null, createdBy);
            }
    ...
    static async updateFlowStepService(flowId, stepNo, statusName, declaration, userId, updatedBy) {
        try {
            const result = await FlowDao.updateFlowStep(flowId, stepNo, statusName, declaration, userId, updatedBy);
    ...
    static async addFlowStepService(flowId, statusName, declaration,userId,createdBy){
            try{
            const result = await FlowDao.addFlowStep(flowId, statusName, declaration, userId,createdBy);
