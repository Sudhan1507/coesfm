...
const UpdatePTW=({row,onSave,onCancel, showAlert})=>{
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const checklistResponse = await axios.get('http://localhost:8080/checklist/names');
        setChecklists(checklistResponse.data.map(item => ({ value: item.checklistId, label: item.cName })));
        const flowResponse = await axios.get('http://localhost:8080/flow/names');
        setFlows(flowResponse.data.map(item => ({ value: item.flowId, label: item.flowName })));
        const reqTagResponse = await axios.get('http://localhost:8080/requestor/names');
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      try {
        const response = await axios.put(`http://localhost:8080/permitType/update/${row.ptId}`, formData);
