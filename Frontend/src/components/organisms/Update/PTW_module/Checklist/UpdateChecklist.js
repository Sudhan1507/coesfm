...
const UpdateChecklist = ({ row, showAlert, onClose }) => {
    ...
    const getChecklistData = useCallback(async () => {
        if (row && row.appId) {
            try {
                const response = await axiosInstance.get(`/ptw/checklistResponse/${row.appId}`);
            ...
            console.log("Row or appId is not defined"); // Debugging
        }
    }, [row]);
    ...
    const handleFormSubmit = async () => {
        ...
        try {
            ...
            const updateResponse = await axiosInstance.put(`/ptw/updateChecklistResponse/${row.appId}`, payload);
