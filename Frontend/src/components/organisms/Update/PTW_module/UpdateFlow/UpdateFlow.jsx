...
import Layout from "../../../../molecules/Layout/Layout.jsx";
...
import DeleteFlowStep from "../../../Delete/PTW_module/DeleteFlow/DeleteFlowStep.jsx";
import Drawer from "../../../../molecules/Drawer/Drawer.jsx";
import Alert from "../../../../atoms/Alert/Alert.jsx";
import AddIcon from "@mui/icons-material/Add";
import CheckCircleOutlineIcon from "@mui/icons-material/CheckCircleOutline";
import UpdateFlowStep from "./UpdateFlowStep.jsx";
import NewFlowStep from "../../../Create/PTW_module/ApprovalFlow/NewFlowStep.jsx";
...
const UpdateFlow=()=>{
    ...
    const getFlowDetails= useCallback(async()=>{
      if(flowId){
     await axiosInstance.get(`/flow/flowDetails/${flowId}`).then((res)=>{
          setData(res.data);
      }).catch((err)=>{
          console.error(err); 
          showAlertHandler({
  ...
  const getFlowName = useCallback(async() => {
    if (flowId) {
       await axiosInstance.get(`/flow/FlowName/${flowId}`).then((res) => {
            console.log("Fetched Flow Name:", res.data.flowName);
...
const handleClear = () => {
  ...
  setValidationError({});
    ...
    const validate = async () => {
      ...
      if (!updateFlowName.trim()) {
        ...
        try {
            ...
            errors.flowName = 'Unable to verify flow name uniqueness. Please try again later.';
        }
    }
    ...
    const handleSubmit = async (e) => {
      ...
      if (await validate()) {
        ...
        try {
          const response = await axiosInstance.put(`/flow/updateFlowName/${flowId}`, formData);
  ...
  const handleSave = () => {
    ...
    showAlertHandler({
      type: 'success',
      message: updateRow ? `Step ${updateRow.stepNo} has been successfully updated.` : `New step has been successfully created`,
      duration: 3000,
  ...
  const getActionCell = (row)=>{
    ...
    return (
        <div className='actions-icon'>
          <button type='button' name='edit' onClick={() => {
            setUpdateRow(row);
            handleOpenDrawer(); 
            }} >
...
const headers=[
    ...
    {key: null, label:'Actions', renderer: getActionCell }
      ...
      return(
        <>
        <Layout>
        {deleteId > 0 && <DeleteFlowStep id={deleteId} onClose={onModalClose} stepNo={deleteStep} showAlert={showAlertHandler} />}
        ...
        <div className='container-body'>
         ...
         <Form className
             ...
             required={true}
          /> 
{validationError.flowName && <div className="error">{validationError.flowName}</div>}
</div>
      ...
      >
        ...
        {updateRow ? (
          <UpdateFlowStep row={updateRow} onSave={handleSave} onCancel={handleCloseDrawer} showAlert={showAlertHandler} />
        ) : (
          <NewFlowStep id={addStep} onSave={handleSave} onCancel={handleCloseDrawer} showAlert={showAlertHandler} />
        )}
      </Drawer>
      {showAlert.show && (
