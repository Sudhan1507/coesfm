...
const NewFlow = ({ onSave, onCancel,showAlert }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setAccount(response.data.map(item => ({ value: item.userId, label: item.displayName })));
            } catch (err) {
    ...
    const handleAddStep = () => {
        setFlowDetails({
            ...flowDetails,
            steps: [...flowDetails.steps, { statusName: '', declaration: '', userId: null }]
        });
    };
    ...
    const handleInsertStep = (index) => {
        const newSteps = [...flowDetails.steps];
        newSteps.splice(index + 1, 0, { statusName: '', declaration: '', userId: null });
        setFlowDetails({
            ...flowDetails,
            steps: newSteps
        });
