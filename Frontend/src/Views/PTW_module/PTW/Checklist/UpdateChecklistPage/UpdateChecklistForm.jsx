  ...
  const handleFormSubmit = async (e) => {
    ...
    if (isRemarksValid) {
      ...
      try {
        ...
        const updateResponse =  await axios.put(`http://localhost:8080/api/ptw/updateChecklistResponse/${decryptedAppId}`, payload);
        if (updateResponse.data.status === 'success') {
          // Proceed to update application status and insert sign off
          await axios.put(`http://localhost:8080/api/ptw/updateAppStatus/${decryptedAppId}`);

          await axios.post(`http://localhost:8080/api/ptw/appSignOff`, {
              appId: decryptedAppId,
        ...
        {data.length > 0 && (
          <>
            <div className="container-header-checklist">
              ...
              <div className="container-header-id-checklist">Checklist ID: {checklistId}</div>
            </div>
            <div className="container-body">
              <form className="checklist-form" onSubmit={handleFormSubmit}>
                ...
                <div className="checklist-footer">
                  <Button label="Submit" size="small" variant="success" type="submit" />
                </div>
              </form>
            </div>
          </>
