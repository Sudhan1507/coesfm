...
import DeleteFlow from '../../../../components/organisms/Delete/PTW_module/DeleteFlow/DeleteFlow.jsx';
import ErrorOutlineOutlinedIcon from '@mui/icons-material/ErrorOutlineOutlined';
import { useNavigate } from 'react-router-dom';
import NewFlow from '../../../../components/organisms/Create/PTW_module/ApprovalFlow/NewFlow.jsx';
...
const ApprovalFlow=()=>{
    ...
    const getActionCell = (row) => {
      return (
        <div className='actions-icon'>
          ...
          navigate('/updateFlow', { state: { flowId: row.flowId, flowName:row.flowName } });
          }}>
    ...
    const getFlowIndex = useCallback(async () => {
     await axiosInstance.get('/flow/flowIndex')
        .then((res) => {
          setData(res.data);
        })
        .catch(err => {
  ...
  const handleSave = () => {
    ...
    return (
      <Layout>
        {deleteId > 0 && <DeleteFlow id={deleteId} onClose={onModalClose} name={deleteName} showAlert={showAlertHandler} />}
        ...
        size='800px'
      >
          <NewFlow onSave={handleSave} onCancel={handleCloseDrawer} showAlert={showAlertHandler} />
      </Drawer>
