...
const Attendance = () => {
  ...
  const handleDelete = (id) => {
    axiosInstance.delete(`/attendance/delete_attendance/${id}`)
      .then(result => {
        if (result.data.Status) {
          // Remove the deleted item from state
          setAttendanceData(prevData => prevData.filter(item => item.id !== id));
          setFilterAttendance(prevData => prevData.filter(item => item.id !== id));
        } else {
  ...
  const handleSearchChange = (e) => {
    ...
    const filteredAttendance = attendanceData.filter((e) =>
                          ...
                          <div className="card-body">
                            <div className="box-header with-border">
                              <div className="box_add">
                                <button>
                                  <Link to='/display/add_attendance'>New Attendance</Link>
                                </button>
                              </div>
                                    ...
                                    {filterAttendance.length ? (
                                      filterAttendance.map(attendance => (
                                        <tr key={attendance.id}>
                                          ...
                                          <td>
                                          <img src={attendance.image} style={{ width: '100px', height: 'auto' }} />
                                          </td>
                                          <td>
                                            <button
                                              title="Edit"
                                              ...
                                              onClick={() => {
                                                ...
                                                if (isConfirmed) {
                                                  navigate(`/display/edit_attendance/${attendance.id}`);
                                                }
                                              }}
                                            >
                                              <HiPencilSquare />
